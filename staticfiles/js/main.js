 ...
 -------------------------------------------------------------------------*/
  ...
  var FixRTL = () => {
    document.addEventListener('DOMContentLoaded', () => {
      // انتخاب همه عناصر با کلاس‌های nav-next-slider و nav-prev-slider
const elements = document.querySelectorAll('.nav-next-slider, .nav-prev-slider');

// ایجاد یک div جدید
const rotatedDiv = document.createElement('div');

// اضافه کردن استایل چرخش به div جدید
rotatedDiv.style.transform = 'rotateY(180deg)';
rotatedDiv.style.display = 'inline-flex'; // اختیاری: برای نمایش مناسب عناصر داخل

// انتقال هر عنصر به div جدید
elements.forEach(element => {
  rotatedDiv.appendChild(element);
});

// اضافه کردن div جدید به DOM (مثلاً به body)
document.body.appendChild(rotatedDiv);
              ...
              case "email-required":
                messageDiv.html(
                  objUse.noticeError.replace(
                    "{msg}",
                    "Error! <strong>تلفن is required.</strong>"
                  )
                );
                break;
              case "email-err":
                messageDiv.html(
                  objUse.noticeError.replace(
                    "{msg}",
                    "Error! <strong>تلفن invalid.</strong>"
                  )
                );
                break;
              case "duplicate":
                messageDiv.html(
                  objUse.noticeError.replace(
                    "{msg}",
                    "Error! <strong>تلفن is duplicate.</strong>"
                  )
                );
                break;
              case "filewrite":
  ...
  function InsetRemover() {
    $('.dropdown-menu.p-0.border-0.show').css('inset', '');
    
    // ذخیره شناسه setInterval در متغیر intervalId
  ...
  let FixLookBook = () => {
    // انتخاب تمام دکمه‌هایی که کلاس tf-pin-btn دارند و اضافه کردن رویداد کلیک به آن‌ها
    if ($('.tf-pin-btn').length) {
      $('.tf-pin-btn').on('click', () => {
        // انتخاب تمام عناصر با کلاس dropdown-menu p-0 border-0 show
        $('.dropdown-menu.p-0.border-0.show').each(function () {
          // حذف ویژگی inset از استایل
          InsetRemover();
        });
      });
    }
  
    // اضافه کردن رویداد اسکرول به window
    $(window).on('scroll', () => {
      // انتخاب تمام عناصر با کلاس dropdown-menu p-0 border-0 show
      if ($('.dropdown-menu.p-0.border-0.show').length) {
        $('.dropdown-menu.p-0.border-0.show').each(function () {
          // حذف ویژگی inset از استایل
          InsetRemover();
        });
    ...
    // اضافه کردن رویداد کلیک به کل صفحه برای خاموش کردن setInterval
    $(document).on('click', () => {
      if (intervalId) {
        clearInterval(intervalId); // متوقف کردن setInterval
        intervalId = null; // تنظیم intervalId به null
      }
    });
  }
  
  // اجرای تابع FixLookBook
  FixLookBook();
  

    
window.addEventListener('load' , function() {
    if ($('.icv__label-after').length) {
      $('.icv__label-after').text('بعد')
      $('.icv__label-before').text('قبل')
    }  
}) 
