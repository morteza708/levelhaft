...
def send_charge_sms(user, amount):
    ...
    message = f"{amount:,}"
    send_message(user.phone_number, message, template='wallet-charge-customer')
...
def send_refund_sms(user, amount, order_number):
    ...
    message = f"{amount:,}"
    send_message(user.phone_number, message, template='wallet-canceled-customer')
...
def send_cancel_notification_to_admin(order):
    ...
    message = f"..."
    send_message(settings.ADMIN_PHONE, message, template='canceled-notification-manager')
